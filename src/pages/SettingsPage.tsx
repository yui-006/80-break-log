...
export function SettingsPage() {
  const { state, exportData, importData, clearAll, saveClub, deleteClub } = useApp();
  ...
  return (
    <div className="min-h-full bg-gray-50">
      ...
      <div className="px-4 py-4 space-y-5">
        {msg && (
          <div className="bg-green-50 border border-green-200 text-green-800 text-sm px-4 py-3 rounded-2xl">
            {msg}
          </div>
        )}
        ...
        <Card className="p-4">
          ...
          <div className="space-y-3">
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 bg-green-800 text-white py-3.5 rounded-2xl font-bold text-base active:bg-green-900"
            >
              <Download size={18} /> JSONエクスポート
            </button>
            <button
              onClick={() => fileRef.current?.click()}
              className="w-full flex items-center justify-center gap-2 bg-gray-100 text-gray-800 py-3.5 rounded-2xl font-bold text-base active:bg-gray-200"
            >
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between mb-3">
            <h2 className="font-bold text-gray-900">クラブセッティング</h2>
            <button
            ...
            {state.clubs.map(c => (
              <div key={c.id} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                <div>
                  ...
                  {c.head && <span className="text-xs text-gray-500 ml-2">{c.head}</span>}
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="text-xs text-green-700 px-2 py-1 rounded-lg bg-green-50"
                  >
                    編集
                  </button>
                  <button
        ...
        <Card className="p-4">
          ...
          {!showClear ? (
            <button
              ...
              className="w-full flex items-center justify-center gap-2 border border-red-200 text-red-600 py-3 rounded-2xl font-medium text-sm active:bg-red-50"
            >
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-3">
                <button
                  onClick={handleClear}
                  className="flex-1 bg-red-600 text-white py-3 rounded-2xl font-bold text-sm active:bg-red-700"
                >
                  削除する
                </button>
                <button
                  onClick={() => setShowClear(false)}
                  className="flex-1 bg-gray-100 text-gray-700 py-3 rounded-2xl font-bold text-sm"
                >
                  キャンセル
                </button>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex flex-col justify-end" onClick={() => setEditingClub(null)}>
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={e => setEditingClub({ ...editingClub, category: e.target.value as Club['category'] })}
              ...
              <div>
                ...
                <input
                  value={editingClub.head ?? ''}
                  onChange={e => setEditingClub({ ...editingClub, head: e.target.value })}
                  className="mt-1 w-full border border-gray-200 rounded-xl px-3 py-2 text-sm"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={e => setEditingClub({ ...editingClub, loft: e.target.value ? Number(e.target.value) : undefined })}
                  className="mt-1 w-full border border-gray-200 rounded-xl px-3 py-2 text-sm"
                  placeholder="°"
                />
              </div>
              <div>
                ...
                <input
                  value={editingClub.shaft ?? ''}
                  onChange={e => setEditingClub({ ...editingClub, shaft: e.target.value })}
                  className="mt-1 w-full border border-gray-200 rounded-xl px-3 py-2 text-sm"
                  placeholder="シャフト名"
                />
              </div>
              <div>
                ...
                <input
                  value={editingClub.flex ?? ''}
                  onChange={e => setEditingClub({ ...editingClub, flex: e.target.value })}
                  className="mt-1 w-full border border-gray-200 rounded-xl px-3 py-2 text-sm"
                  placeholder="R / S / X"
                />
              </div>
            ...
            <div className="flex gap-3">
              <button
                onClick={async () => {
                  if (!editingClub.name.trim()) return;
                  await saveClub(editingClub);
                  setEditingClub(null);
                }}
                disabled={!editingClub.name.trim()}
                className="flex-1 bg-green-800 text-white py-3 rounded-2xl font-bold disabled:opacity-40"
              >
                保存
              </button>
              <button
